/* eslint-disable*/
// clone from https://github.com/YousefED/typescript-json-schema/blob/master/typescript-json-schema.ts
    ...
    if (!asRef || !this.reffedDefinitions[fullTypeName]) {
      if (asRef) {
        ...
        if (asTypeAliasRef && reffedType && typ.symbol !== reffedType && symbol) {
          reffedDefinition = this.getTypeDefinition(typ, true, undefined, symbol, symbol, undefined, forceNotRef);
        } else {
      ...
      if (definition.type === undefined) {
        ...
        if (typ.flags & ts.TypeFlags.Union) {
          ...
          node && (node.kind === ts.SyntaxKind.EnumDeclaration || node.kind === ts.SyntaxKind.EnumMember)
        ) {
          this.getEnumDefinition(typ, definition);
        } else if (
          symbol && symbol.flags & ts.SymbolFlags.TypeLiteral &&
